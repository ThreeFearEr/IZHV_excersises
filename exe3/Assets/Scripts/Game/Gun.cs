...
public class Gun : MonoBehaviour
{
    ...
    { 
        ...
        bin = GameObject.Find("Bin").transform;
    ...
    public void ShootGun(Transform director)
    {
        ...
        if(shotgun){
            ...
            for (int i = 0; i < shotgunBullets; i++){
                float randomOffset = UnityEngine.Random.Range(-shotgunSpread / 2f, shotgunSpread / 2f);
                Quaternion randomRotation = Quaternion.Euler(0, 0, randomOffset);
                SpawnBullet(Vector3.zero, randomRotation);
            }
        }
        else{
            SpawnBullet(
    ...
    public void SpawnBullet(Vector3 position, Quaternion rotation)
    {
        ...
        { // Using default -> Spawn new GameObject.
            ...
            var bullet = Instantiate(bulletPrefab, firePoint.position, firePoint.rotation * rotation, bin);
            if(player.mMoveInput.x != 0 || player.mMoveInput.y != 0) bullet.GetComponent<Bullet>().speed += player.speed;
